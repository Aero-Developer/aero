 ...
 * string, which is used as given - so an `api.etherscan.io/v2` URL with the user's own key works,
 * as does a private Blockscout.
 */
void aero_set_history_apis(uint64_t chain_id, const char *csv);
...
char *aero_wallet_hl_withdraw(AeroWallet *w,
                              ...
                              const char *amount_usdc);
/**
 * What redeeming `amount_xmr1` of XMR1 for real Monero would cost: Wagyu's fee, its minimum, and
 * the net payout. Read live, since the fee is Wagyu's runtime setting rather than a constant.
 * Caller frees the string.
 */
...
/**
 * Redeem XMR1 for Monero paid to `monero_address`. Opens a Wagyu order, then sends the XMR1 to the
 * Hyperliquid address that order names. Returns the order id and session id needed to track it.
 * Caller frees the string.
 */
